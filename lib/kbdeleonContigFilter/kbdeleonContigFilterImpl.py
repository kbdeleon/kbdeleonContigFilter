...
This sample module contains one small method that filters contigs.
    ...
    def run_kbdeleonContigFilter(self, ctx, params):
        """
        ...
        #BEGIN run_kbdeleonContigFilter

        # Print statements to stdout/stderr are captured and available as the App log
        logging.info('Starting run_kbdeleonContigFilter function. Params=' + pformat(params))
        ...
        if max_length > 999999999:
            raise ValueError('max_length parameter cannot be greater than 999,999,999 (' + str(max_length) + ')')
        if max_length < min_length:
            raise ValueError('max_length ('+ str(max_length)+') cannot be less than min_length (' + str(min_length) + ')')
        ...
        #END run_kbdeleonContigFilter

        # At some point might do deeper type checking...
        if not isinstance(output, dict):
            raise ValueError('Method run_kbdeleonContigFilter return value ' +
                             'output is not type dict as required.')
        # return the results
        return [output]

    def run_kbdeleonContigFilter_max(self, ctx, params):
        """
        :param params: instance of mapping from String to unspecified object
        :returns: instance of type "ReportResults" -> structure: parameter
           "report_name" of String, parameter "report_ref" of String
        ...
        # ctx is the context object
        # return variables are: output
        #BEGIN run_kbdeleonContigFilter_max
        ...
        #END run_kbdeleonContigFilter_max

        # At some point might do deeper type checking...
        if not isinstance(output, dict):
            raise ValueError('Method run_kbdeleonContigFilter_max return value ' +
                             'output is not type dict as required.')
        # return the results
        return [output]
    def status(self, ctx):
